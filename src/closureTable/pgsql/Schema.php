...
class Schema extends \nineinchnick\closureTable\Schema
{
	...
	{
		$treeTable = (!empty($schemaName) ? '"'.$schemaName.'".' : '').'"'.$this->getTreeTableName($tableName, $tableNameSuffix).'"';
		$tableName = (!empty($schemaName) ? '"'.$schemaName.'".' : '').'"'.$tableName.'"';
		$query = <<<SQL
...
SQL;
	...
	{
		$treeTable = (!empty($schemaName) ? '"'.$schemaName.'".' : '').'"'.$this->getTreeTableName($tableName, $tableNameSuffix).'"';
		$schemaName = (!empty($schemaName) ? '"'.$schemaName.'".' : '');
		$queries = array();
		$queries[] = <<<SQL
-- --------------------------------------------------------------------
-- INSERT:
...
-- --------------------------------------------------------------------
CREATE OR REPLACE FUNCTION {$schemaName}"{$tableName}_tree_ai"() RETURNS TRIGGER AS
\$BODY\$
BEGIN
...
CREATE OR REPLACE FUNCTION {$schemaName}"${tableName}_tree_bu"() RETURNS TRIGGER AS
\$BODY\$
BEGIN
...
SQL;
		$queries[] = <<<SQL
CREATE OR REPLACE FUNCTION {$schemaName}"{$tableName}_tree_au"() RETURNS TRIGGER AS
\$BODY\$
BEGIN
  IF OLD."$parentKey" IS NOT DISTINCT FROM NEW."$parentKey" THEN
    RETURN NEW;
...
CREATE OR REPLACE FUNCTION {$schemaName}"{$tableName}_tree_bi_path"() RETURNS TRIGGER AS
\$BODY\$
BEGIN
  IF NEW."$parentKey" IS NULL THEN
...
SQL;
			$queries[] = <<<SQL
CREATE OR REPLACE FUNCTION {$schemaName}"{$tableName}_tree_bu_path"() RETURNS TRIGGER AS
\$BODY\$
DECLARE
  replace_from TEXT := '^';
  replace_to TEXT := '';
...
SQL;
		...
		$queries[] = "CREATE TRIGGER \"{$tableName}_tree_ai\" AFTER INSERT ON {$schemaName}\"$tableName\" FOR EACH ROW EXECUTE PROCEDURE {$schemaName}\"{$tableName}_tree_ai\"()";
		$queries[] = "CREATE TRIGGER \"{$tableName}_tree_bu\" BEFORE UPDATE ON {$schemaName}\"$tableName\" FOR EACH ROW EXECUTE PROCEDURE {$schemaName}\"{$tableName}_tree_bu\"()";
		$queries[] = "CREATE TRIGGER \"{$tableName}_tree_au\" AFTER UPDATE ON {$schemaName}\"$tableName\" FOR EACH ROW EXECUTE PROCEDURE {$schemaName}\"{$tableName}_tree_au\"()";
		if ($path !== null) {
			$queries[] = "CREATE TRIGGER \"{$tableName}_tree_bi_path\" BEFORE INSERT ON {$schemaName}\"$tableName\" FOR EACH ROW EXECUTE PROCEDURE {$schemaName}\"{$tableName}_tree_bi_path\"()";
			$queries[] = "CREATE TRIGGER \"{$tableName}_tree_bu_path\" BEFORE UPDATE ON {$schemaName}\"$tableName\" FOR EACH ROW EXECUTE PROCEDURE {$schemaName}\"{$tableName}_tree_bu_path\"()";
		}
		return $queries;
	}
